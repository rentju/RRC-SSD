...
#设置value大小控制雨滴数量，length大小控制雨水水痕长度，angle大小来控制雨下落的角度，w来控制雨点粗细程度 要是奇数。
for jpgfile in glob.glob(r'D:\kitti\mini\original\*.png'):
#for jpgfile in glob.glob(r'D:\PYcode\opencv\test1\*.png'):
    img = cv2.imread(jpgfile)
    noise = get_noise(img, value=70)
    rain = rain_blur(noise, length=5, angle=-10, w=5)
    addrain=alpha_rain(rain, img, beta=0.9)  # 方法一，透明度赋值
    # add_rain(rain,img)    #方法二,加权后有玻璃外的效果
    cv2.imwrite(os.path.join(r'D:\kitti\mini\snow', os.path.basename(jpgfile)), addrain)
